...
namespace FYPMSWebsite.App_Code
{
    ...
    public class FYPMSDB
    {
        ...
        public DataTable GetProjectsWithoutReaders()
        {
            ...
            //**************************************************************************
            sql = "SELECT P.groupId,P.groupCode,F.title,F.fypCategory,F.fypType FROM ProjectGroup P ,FYProject F WHERE P.fypAssigned = F.fypId and P.reader is null order by P.groupCode asc";
        ...
        public DataTable GetStudentRequirements(string groupId, string fypId)
        {
            ...
            //****************************************************************************************
            sql = "select distinct s.username,s.studentname,r.proposalgrade,r.progressgrade,r.finalgrade,r.presentationgrade from requirement R,Students S,Projectgroup G,Supervises sp,Faculty F where R.studentusername = S.username and s.groupid = g.groupId and g.fypassigned = sp.fypid and r.facultyusername = sp.username and s.groupid = "+groupId+" and sp.fypid = "+fypId;
            return myOracleDBAccess.GetData(sql);
        }
